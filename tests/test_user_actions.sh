...
# Create a file with a wrong format
echo -ne "\nCreating a file with a wrong format, press enter to continue..."
read press_enter

message=$(curl -s $server/$1/$3 -H "Authorization: token $token" -d '{doc_content: {"fruit": "Apple", "size": "Large", "color": "Red"}}' -X POST)
echo $message
